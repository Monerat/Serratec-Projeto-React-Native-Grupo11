...
interface DigimonListResponse extends AxiosResponse<{ count: number; results: Digimon[] }, any> {}

export function getDigimonDetails(idOrName: string): Promise<DigimonDetailsResponse> {
  const url = `digimon/${idOrName}`;
  return apiDigimon.get(url);
}

export function getDigimonList(name?: string, attribute?: string, xAntibody?: boolean, level?: string, page?: number, pageSize?: number): Promise<DigimonListResponse> {
  const url = 'digimon';
